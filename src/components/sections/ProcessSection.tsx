...
const steps = [{
  number: 1,
  text: "Você fala conosco pelo WhatsApp",
  icon: MessageCircle
}, {
  number: 2,
  text: "Nossa equipe faz a primeira orientação",
  icon: Phone
}, {
  number: 3,
  text: "Agendamos a consulta",
  icon: Calendar
}, {
  number: 4,
  text: "Seu pet passa por avaliação especializada",
  icon: Stethoscope
}, {
  number: 5,
  text: "Você recebe diagnóstico e direcionamento de tratamento",
  icon: FileCheck
}];
const fadeInUp = {
  hidden: {
...
const ProcessSection = () => {
  return <section id="processo" className="section-padding bg-white relative overflow-hidden" aria-labelledby="process-heading">
      {/* Decorative elements */}
      ...
      <div className="container-narrow relative z-10">
        <motion.h2 id="process-heading" className="heading-lg text-foreground text-center mb-12 font-sans" initial={{
        opacity: 0,
        y: 20
      }} whileInView={{
        opacity: 1,
        ...
        <motion.div className="relative max-w-2xl mx-auto" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{
        once: true
      }}>
          {/* Connecting line */}
          ...
          <div className="space-y-6">
            {steps.map((step, index) => <motion.div key={index} className="flex items-center gap-6 relative" variants={fadeInUp} transition={{
            duration: 0.4
          }}>
